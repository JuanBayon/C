#include <stdio.h>
#include <assert.h>
#include "csv.h"
#include "api.h"

#include <string.h>
#include "person.h"
#include "vaccine.h"


#define FILE_READ_BUFFER_SIZE 2048

// Get the API version information
const char* api_version() {
    return "UOC PP 20212";
}

// Load data from a CSV file. If reset is true, remove previous data
tApiError api_loadData(tApiData* data, const char* filename, bool reset) {
    tApiError error;
    FILE *fin;    
    char buffer[FILE_READ_BUFFER_SIZE];
    tCSVEntry entry;
    
    // Check input data
    assert( data != NULL );
    assert(filename != NULL);
    
    // Reset current data    
    if (reset) {
        // Remove previous information
        error = api_freeData(data);
        if (error != E_SUCCESS) {
            return error;
        }
        
        // Initialize the data
        error = api_initData(data);
        if (error != E_SUCCESS) {
            return error;
        }
    }

    // Open the input file
    fin = fopen(filename, "r");
    if (fin == NULL) {
        return E_FILE_NOT_FOUND;
    }
    
    // Read file line by line
    while (fgets(buffer, FILE_READ_BUFFER_SIZE, fin)) {
        // Remove new line character     
        buffer[strcspn(buffer, "\n\r")] = '\0';
        
        csv_initEntry(&entry);
        csv_parseEntry(&entry, buffer, NULL);
        // Add this new entry to the api Data
        error = api_addDataEntry(data, entry);
        if (error != E_SUCCESS) {
            return error;
        }
        csv_freeEntry(&entry);
    }
    
    fclose(fin);
    
    return E_SUCCESS;
}

// Initialize the data structure
tApiError api_initData(tApiData* data) {            
    //////////////////////////////////
    // Ex PR1 2b
    /////////////////////////////////
    // Check input data structure
    assert(data != NULL);
    
    // Initialize data structures
    population_init(&(data->population));
    vaccineList_init(&(data->vaccines));
    vaccineLotData_init(&(data->vaccineLots));
    
    
    //////////////////////////////////
    // Ex PR1 2b
    /////////////////////////////////
    centerList_init(&(data->centers));
    /////////////////////////////////
    
    return E_SUCCESS;
    
    /////////////////////////////////
    // return E_NOT_IMPLEMENTED;
}

// Add a new vaccines lot
tApiError api_addVaccineLot(tApiData* data, tCSVEntry entry) {
    //////////////////////////////////
    // Ex PR1 2c
    /////////////////////////////////
    tVaccine vaccine;
    tVaccineLot lot;
    tVaccine *pVaccine;
    
    //////////////////////////////////
    // Ex PR2 3c
    /////////////////////////////////
    tHealthCenter *pCenter;
    /////////////////////////////////
    
    // Check input data structure
    assert(data != NULL);
    
    // Check the entry type
    if (strcmp(csv_getType(&entry), "VACCINE_LOT") != 0) {
        return E_INVALID_ENTRY_TYPE;
    }
    
    // Check the number of fields
    if(csv_numFields(entry) != 7) {
        return E_INVALID_ENTRY_FORMAT;
    }
    
    // Parse the entry
    vaccineLot_parse(&vaccine, &lot, entry);
    
    // Check if vaccine exists
    pVaccine = vaccineList_find(data->vaccines, vaccine.name);
    if (pVaccine == NULL) {
        // Add the vaccine
        vaccineList_insert(&(data->vaccines), vaccine);
        pVaccine = vaccineList_find(data->vaccines, vaccine.name);
    }
    assert(pVaccine != NULL);
    
    // Assign this vaccine to the lot
    lot.vaccine = pVaccine;
    
    // Add the lot to the data
    vaccineLotData_add(&(data->vaccineLots), lot);
    
    
    //////////////////////////////////
    // Ex PR2 3c
    /////////////////////////////////
    pCenter = centerList_find(&(data->centers), lot.cp);
    if (pCenter == NULL) {
        centerList_insert(&(data->centers), lot.cp);
        pCenter = centerList_find(&(data->centers), lot.cp);
    }
    stockList_update(&(pCenter->stock), lot.timestamp.date, lot.vaccine, lot.doses);
    /////////////////////////////////
    
    
    // Release temporal data
    vaccine_free(&vaccine);
    vaccineLot_free(&lot);
    
    return E_SUCCESS;
    
    /////////////////////////////////
    
    //return E_NOT_IMPLEMENTED;
}

// Get the number of persons registered on the application
int api_populationCount(tApiData data) {
    //////////////////////////////////
    // Ex PR1 2d
    /////////////////////////////////
    return population_len(data.population);
    /////////////////////////////////
    //return -1;
}

// Get the number of vaccines registered on the application
int api_vaccineCount(tApiData data) {
    //////////////////////////////////
    // Ex PR1 2d
    /////////////////////////////////
    return vaccineList_len(data.vaccines);    
    /////////////////////////////////
    //return -1;
}

// Get the number of vaccine lots registered on the application
int api_vaccineLotsCount(tApiData data) {
    //////////////////////////////////
    // Ex PR1 2d
    /////////////////////////////////
    return vaccineLotData_len(data.vaccineLots);
    /////////////////////////////////
    //return -1;
}


// Free all used memory
tApiError api_freeData(tApiData* data) {
    //////////////////////////////////
    // Ex PR1 2e
    /////////////////////////////////
    population_free(&(data->population));
    vaccineLotData_free(&(data->vaccineLots));
    vaccineList_free(&(data->vaccines));
    
    //////////////////////////////////
    // Ex PR2 3d
    /////////////////////////////////
    centerList_free(&(data->centers));
    /////////////////////////////////
    
    return E_SUCCESS;
    /////////////////////////////////
    //return E_NOT_IMPLEMENTED;
}


// Add a new entry
tApiError api_addDataEntry(tApiData* data, tCSVEntry entry) { 
    //////////////////////////////////
    // Ex PR1 2f
    /////////////////////////////////
    int personIdx;
    tPerson person;
        
    assert(data != NULL);
    
    // Initialize the person object
    person_init(&person);
        
    if (strcmp(csv_getType(&entry), "PERSON") == 0) {
        // Check the number of fields
        if(csv_numFields(entry) != 7) {
            return E_INVALID_ENTRY_FORMAT;
        }
        // Parse the data
        person_parse(&person, entry);
        
        // Check if this person already exists
        if (population_find(data->population, person.document) >= 0) {
            // Release person object
            person_free(&person);
            return E_DUPLICATED_PERSON;
        }
        
        // Add the new person
        population_add(&(data->population), person);
        
        // Release person object
        person_free(&person);
        
    } else if (strcmp(csv_getType(&entry), "VACCINE_LOT") == 0) {
        return api_addVaccineLot(data, entry);        
    } else {
        return E_INVALID_ENTRY_TYPE;
    }
    return E_SUCCESS;
    /////////////////////////////////
    //return E_NOT_IMPLEMENTED;
}

// Get vaccine data
tApiError api_getVaccine(tApiData data, const char *name, tCSVEntry *entry) {
    //////////////////////////////////
    // Ex PR1 3a
    /////////////////////////////////
    char buffer[2048];
    tVaccine* vaccine = NULL;
        
    assert(name != NULL);
    assert(entry != NULL);
    
    // Search vaccine
    vaccine = vaccineList_find(data.vaccines, name);
    
    if (vaccine == NULL) {
        return E_VACCINE_NOT_FOUND;
    }
    
    // Print data in the buffer
    sprintf(buffer, "%s;%d;%d", vaccine->name, vaccine->required, vaccine->days);
    
    // Initialize the output structure
    csv_initEntry(entry);
    csv_parseEntry(entry, buffer, "VACCINE");
    
    return E_SUCCESS;
    
    /////////////////////////////////
    //return E_NOT_IMPLEMENTED; 
}

// Get vaccine lot data
tApiError api_getVaccineLot(tApiData data, const char* cp, const char* vaccine, tDateTime timestamp, tCSVEntry *entry) {
    //////////////////////////////////
    // Ex PR1 3b
    /////////////////////////////////
    char buffer[2048];
    int idx;
        
    assert(cp != NULL);
    assert(vaccine != NULL);
    assert(entry != NULL);
    
    // Search vaccine
    idx = vaccineLotData_find(data.vaccineLots, cp, vaccine, timestamp);
        
    if (idx < 0) {
        return E_LOT_NOT_FOUND;
    }
    
    // Print data in the buffer
    sprintf(buffer, "%02d/%02d/%04d;%02d:%02d;%s;%s;%d;%d;%d", 
        data.vaccineLots.elems[idx].timestamp.date.day, data.vaccineLots.elems[idx].timestamp.date.month, data.vaccineLots.elems[idx].timestamp.date.year,
        data.vaccineLots.elems[idx].timestamp.time.hour, data.vaccineLots.elems[idx].timestamp.time.minutes,
        data.vaccineLots.elems[idx].cp,
        data.vaccineLots.elems[idx].vaccine->name, data.vaccineLots.elems[idx].vaccine->required, data.vaccineLots.elems[idx].vaccine->days,
        data.vaccineLots.elems[idx].doses
    );
    
    // Initialize the output structure
    csv_initEntry(entry);
    csv_parseEntry(entry, buffer, "VACCINE_LOT");
    
    return E_SUCCESS;
    
    
    /////////////////////////////////
    //return E_NOT_IMPLEMENTED; 
}

// Get registered vaccines
tApiError api_getVaccines(tApiData data, tCSVData *vaccines) {
    //////////////////////////////////
    // Ex PR1 3c
    /////////////////////////////////
    char buffer[2048];
    tVaccineNode *pNode = NULL;
    
    csv_init(vaccines);
        
    pNode = data.vaccines.first;
    while(pNode != NULL) {
        sprintf(buffer, "%s;%d;%d", pNode->vaccine.name, pNode->vaccine.required, pNode->vaccine.days);
        csv_addStrEntry(vaccines, buffer, "VACCINE");
        pNode = pNode->next;
    }    
    
    return E_SUCCESS;
    /////////////////////////////////
    //return E_NOT_IMPLEMENTED; 
}

// Get vaccine lots
tApiError api_getVaccineLots(tApiData data, tCSVData *lots) {
    //////////////////////////////////
    // Ex PR1 3d
    /////////////////////////////////
    char buffer[2048];
    int idx;
    
    csv_init(lots);
    for(idx=0; idx<data.vaccineLots.count ; idx++) {
        sprintf(buffer, "%02d/%02d/%04d;%02d:%02d;%s;%s;%d;%d;%d", 
            data.vaccineLots.elems[idx].timestamp.date.day, data.vaccineLots.elems[idx].timestamp.date.month, data.vaccineLots.elems[idx].timestamp.date.year,
            data.vaccineLots.elems[idx].timestamp.time.hour, data.vaccineLots.elems[idx].timestamp.time.minutes,
            data.vaccineLots.elems[idx].cp,
            data.vaccineLots.elems[idx].vaccine->name, data.vaccineLots.elems[idx].vaccine->required, data.vaccineLots.elems[idx].vaccine->days,
            data.vaccineLots.elems[idx].doses
        );
        csv_addStrEntry(lots, buffer, "VACCINE_LOT");
    }
    
    return E_SUCCESS;
    
    /////////////////////////////////
    //return E_NOT_IMPLEMENTED; 
}

// Get the number of health centers registered on the application
int api_centersCount(tApiData data) {
    //////////////////////////////////
    // Ex PR2 3e
    
    return data.centers.count;
    
    /////////////////////////////////
    // return -1;
}

// Print center stock
void api_printCenterStock(tApiData data, const char* cp) {
    tHealthCenter *pCenter;
    
    // Check input data    
    assert(cp != NULL);
    
    // Search the health center
    pCenter = centerList_find(&(data.centers), cp);
    if (pCenter != NULL) {
        printf("==============================\n");
        printf("STOCK FOR CENTER %s\n", cp);
        printf("==============================\n");
        stockList_print(pCenter->stock);
        printf("==============================\n\n");
    }    
}

// Add a new vaccination appointment
tApiError api_addAppointment(tApiData* data, const char* cp, const char* document, const char* vaccine, tDateTime timestamp) {
    assert(data != NULL);
    assert(cp != NULL);
    assert(vaccine != NULL);
    assert(document != NULL);

    tPerson* person;
    tVaccine* pVaccine;
    tHealthCenter* center;
    int personIndex;

    personIndex = population_find(data->population, document);
    pVaccine = vaccineList_find(data->vaccines, vaccine);
    center = centerList_find(&data->centers, cp);

    // Check if the person exists
    if (personIndex == -1) {
        return E_PERSON_NOT_FOUND;
    } else {
        person = &(data->population.elems[personIndex]);
    }
    if (pVaccine == NULL) return E_VACCINE_NOT_FOUND;
    if (center == NULL) return E_HEALTH_CENTER_NOT_FOUND;

    // Add the appointment
    appointmentData_insert(&(center->appointments), timestamp, pVaccine, person);
    // Add the second appointment if necessary
    if (pVaccine->required > 1) {
        for (int i = 2; i <= pVaccine->required; i++) {
            dateTime_addDay(&timestamp, pVaccine->days);
            appointmentData_insert(&(center->appointments), timestamp, pVaccine, person);
        }
    }
    return E_SUCCESS; 
}

// Get person appointments
tApiError api_getPersonAppointments(tApiData data, const char* document, tCSVData *appointments) {
    assert(appointments != NULL);

    int firstAppointment;
    int secondAppointment;
    int personIndex;
    tPerson* person;
    tHealthCenterNode *pNode;
    bool found;
    char appointment[FILE_READ_BUFFER_SIZE];

    // Check if the person exists
    personIndex = population_find(data.population, document);
    if (personIndex == -1) {
        return E_PERSON_NOT_FOUND;
    } else {
        person = &(data.population.elems[personIndex]);
    }
    pNode = data.centers.first;
    found = false;

    // Traverse the centers
    while(pNode != NULL && !found) {
        firstAppointment = appointmentData_find(pNode->elem.appointments, person, 0);
        // If a first appointment is found add it to the CSV
        if (firstAppointment != -1) {
            found = true;
            sprintf(appointment, "%02d/%02d/%04d;%02d:%02d;%s;%s", 
                    pNode->elem.appointments.elems[firstAppointment].timestamp.date.day,
                    pNode->elem.appointments.elems[firstAppointment].timestamp.date.month,
                    pNode->elem.appointments.elems[firstAppointment].timestamp.date.year,
                    pNode->elem.appointments.elems[firstAppointment].timestamp.time.hour,
                    pNode->elem.appointments.elems[firstAppointment].timestamp.time.minutes,
                    pNode->elem.cp, pNode->elem.appointments.elems[firstAppointment].vaccine->name);
            csv_addStrEntry(appointments, appointment, "APPOINTMENT");
            // Search for the second appointment
            secondAppointment = appointmentData_find(pNode->elem.appointments, person, firstAppointment + 1);
            // if the second appointment is found add it to the CSV
            if (secondAppointment != -1) {
                sprintf(appointment, "%02d/%02d/%04d;%02d:%02d;%s;%s", 
                        pNode->elem.appointments.elems[secondAppointment].timestamp.date.day,
                        pNode->elem.appointments.elems[secondAppointment].timestamp.date.month,
                        pNode->elem.appointments.elems[secondAppointment].timestamp.date.year,
                        pNode->elem.appointments.elems[secondAppointment].timestamp.time.hour,
                        pNode->elem.appointments.elems[secondAppointment].timestamp.time.minutes,
                        pNode->elem.cp, pNode->elem.appointments.elems[secondAppointment].vaccine->name);
                csv_addStrEntry(appointments, appointment, "APPOINTMENT");
            }
        }
        pNode = pNode->next;        
    }
    return E_SUCCESS; 
}

// Check availability of a vaccine in a given health center
bool api_checkAvailability(tApiData data, const char* cp, const char* vaccine, tDate date) {
    assert(cp != NULL);

    tHealthCenter* center;
    tVaccineDailyStock* stock;
    tVaccineStockNode* node;
    tVaccine* pVaccine;
    bool result = false;

    center = centerList_find(&data.centers, cp);
    if (center == NULL) return result;

    //find stock for the date
    stock = stockList_find(&center->stock, date);
    if (stock != NULL) {
        //If not null find vaccine
        pVaccine = vaccineList_find(data.vaccines, vaccine);
        node = dailyStock_find(stock, pVaccine);
        if (node != NULL) {
            // If the required vaccine needs only one dose the result is true              
            if (pVaccine->required == 1 && node->elem.doses >= pVaccine->required) {
                result = true;
            // If it needs two doses
            } else if (pVaccine->required == 2) {
                date_addDay(&date, pVaccine->days);
                stock = stockList_find(&center->stock, date);
                // If there is no stock for the date, it is checked the last day
                if (stock == NULL) {
                    stock = stockList_find(&center->stock, center->stock.last->day);
                }
                // If there are two doses the result is true
                node = dailyStock_find(stock, pVaccine);
                if (node != NULL && node->elem.doses >= pVaccine->required) {
                    result = true;
                }
            }
        }
    }
    return result;
}

// Find available vaccination appointment
tApiError api_findAppointmentAvailability(tApiData* data, const char* cp, const char* document, tDateTime timestamp) {
    assert(data != NULL);
    assert(cp != NULL);
    assert(document != NULL);

    tVaccineDailyStock* stock;
    tCSVData appointments;
    tHealthCenter* center;
    tVaccineStockNode* node;
    tVaccineNode* vaccineOfAppointment;
    tDateTime dayOfAppointment;
    tPerson* person;
    int lotIndex;

    bool found = false;
    int days = 0;
    tVaccineNode* vaccineNode = data->vaccines.first;

    center = centerList_find(&data->centers, cp);
    if (center == NULL) return E_HEALTH_CENTER_NOT_FOUND;
    if (population_find(data->population, document) == -1) return E_PERSON_NOT_FOUND;

    // Check if the person has already an appointment
    csv_init(&appointments);
    api_getPersonAppointments(*data, document, &appointments);
    if (csv_numEntries(appointments) > 0) {
        csv_free(&appointments);
        return E_DUPLICATED_PERSON;
    } 
    csv_free(&appointments);

    // Search for the day and next 6 days
    while (!found && days < 7) {
        // Search for each vaccine
        while (!found && vaccineNode != NULL) {
            found = api_checkAvailability(*data, cp, vaccineNode->vaccine.name, timestamp.date);
            // If found add appointment
            if (found) {
                api_addAppointment(data, cp, document, vaccineNode->vaccine.name, timestamp);
                vaccineOfAppointment = vaccineNode;
                dayOfAppointment = timestamp;
                stock = stockList_find(&center->stock, timestamp.date);
            }
            vaccineNode = vaccineNode->next;
        }
        dateTime_addDay(&timestamp, 1);
        days++;
    }

    if (!found) {
        days = 0;
        vaccineNode = data->vaccines.first;
        // If not found search the next 7 days
        while (!found && days < 7) {
            stock = stockList_find(&center->stock, timestamp.date);
            // Search for each vaccine
            while (!found && stock != NULL && vaccineNode != NULL) {
                node = dailyStock_find(stock, &vaccineNode->vaccine);
                // if there are one dose at least the appointment is recorded
                if (node != NULL && node->elem.doses >= 1) {
                    api_addAppointment(data, cp, document, vaccineNode->vaccine.name, timestamp);
                    found = true;
                    vaccineOfAppointment = vaccineNode;
                    dayOfAppointment = timestamp;
                }
                vaccineNode = vaccineNode->next;
            }
            dateTime_addDay(&timestamp, 1);
            days++;
        }
    }
    if (!found) return E_NO_VACCINES;

    // Update the stock for the first and the second dose if it exists
    dailyStock_update(stock, &vaccineOfAppointment->vaccine, -1);
    if (vaccineOfAppointment->vaccine.required > 1) {
        dateTime_addDay(&dayOfAppointment, vaccineOfAppointment->vaccine.days);
        stock = stockList_find(&center->stock, dayOfAppointment.date);
        // If there is stock on the day of the last dose
        if (stock != NULL) {
            dailyStock_update(stock, &vaccineNode->vaccine, -1);
        // Otherwise update the last day
        } else {
            stockList_expandRight(&center->stock, dayOfAppointment.date);
            stock = stockList_find(&center->stock, dayOfAppointment.date);
            dailyStock_update(stock, &vaccineOfAppointment->vaccine, -1);
        }
    }

    return E_SUCCESS; 
}
